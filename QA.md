...
**Coverage gate:** `backend/pytest.ini` enforces **`--cov-fail-under=76`** on the full backend suite (line+branch total; full suite is **~80%**; stretch **~90%**). GitHub Actions job **Backend — full suite + coverage gate** runs the same command and uploads **`backend-coverage-xml`** (`coverage.xml`) for dashboards or Codecov. The QA master script adds **`--cov-fail-under=0`** so `npm run qa:all` can finish and write `backend-coverage.json` for the HTML dashboard while you iterate.
